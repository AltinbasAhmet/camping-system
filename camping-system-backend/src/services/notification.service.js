...
// Madde: bilet satıldığında etkinliği düzenleyen kişiye (kamp sahibi ya da
// kampçı) de bilgi gider.
async function notifyOrganizerTicketSold(booking) {
  ...
  const subject = `Yeni bilet satışı: ${booking.event.title}`;
  const text =
    `Merhaba ${organizer.name},\n\n` +
    `"${booking.event.title}" etkinliğiniz için yeni bir bilet satışı oldu.\n\n` +
    `Alıcı: ${buyer?.name || "-"}\n` +
    `Bilet adedi: ${booking.guestCount}\n` +
    `Toplam tutar: ${(booking.event.price * booking.guestCount).toLocaleString("tr-TR")} TL\n` +
    `Etkinlik tarihi: ${formatDateTr(booking.event.dateTime)}`;
  const sms = `CampPal: "${booking.event.title}" etkinliginize ${booking.guestCount} biletlik yeni satis oldu.`;
...
// Madde: check-in tarihinden bir gün önce hatırlatma.
async function notifyUpcomingCheckIn(reservation) {
  const user = reservation.user;

  if (!user) return;

  const subject = `Kampınız yarın başlıyor: ${reservation.reservationCode}`;
  const text =
    `Merhaba ${user.name},\n\n` +
    `${reservation.camp.name} kampındaki rezervasyonunuz yarın (${formatDateOnlyTr(reservation.checkInDate)}) başlıyor.\n\n` +
    `Rezervasyon kodu: ${reservation.reservationCode}\n` +
    `Kamp: ${reservation.camp.name} (${reservation.camp.city})\n` +
    `Adres: ${reservation.camp.address}\n\n` +
    `İyi kamplar dileriz!`;
  const sms = `CampPal: ${reservation.camp.name} kampindaki rezervasyonunuz (${reservation.reservationCode}) yarin basliyor.`;
...
// Madde: check-out tarihinden bir gün önce hatırlatma.
async function notifyUpcomingCheckOut(reservation) {
  const user = reservation.user;

  if (!user) return;

  const subject = `Kampınız yarın sona eriyor: ${reservation.reservationCode}`;
  const text =
    `Merhaba ${user.name},\n\n` +
    `${reservation.camp.name} kampındaki rezervasyonunuz yarın (${formatDateOnlyTr(reservation.checkOutDate)}) sona eriyor.\n\n` +
    `Rezervasyon kodu: ${reservation.reservationCode}\n\n` +
    `İyi yolculuklar dileriz!`;
  const sms = `CampPal: ${reservation.camp.name} kampindaki rezervasyonunuz (${reservation.reservationCode}) yarin sona eriyor.`;
