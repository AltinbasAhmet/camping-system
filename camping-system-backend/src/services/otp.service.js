...
async function generateAndSendOtp(user, purpose, notificationChannel) {
  ...
  const purposeText = purpose === "REGISTER" ? "hesabınızı doğrulamak" : purpose === "RESET_PASSWORD" ? "şifrenizi yenilemek" : "giriş yapmak";
  const subject = purpose === "RESET_PASSWORD" ? "CampPal şifre yenileme kodunuz" : "CampPal doğrulama kodunuz";
  const text =
    `Merhaba ${user.name},\n\n` +
  ...
  if (notificationChannel === "EMAIL") {
    ...
    if (result.skipped) throw new AppError("E-posta gönderilemedi. Lütfen SMTP ayarlarını kontrol edin.", 503);
    ...
    if (result.skipped) throw new AppError("SMS gönderilemedi. Lütfen Twilio ayarlarını ve hedef ülke iznini kontrol edin.", 503);
  } else {
...
async function verifyOtp(userId, code, expectedPurpose) {
  ...
  if (expectedPurpose && purpose !== expectedPurpose) {
    throw new AppError("Doğrulama kodu bu işlem için geçerli değil", 400);
  }
