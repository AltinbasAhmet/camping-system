...
if (demoEmails.includes(user.email)) {
  ...
  return {
    ...
    otpRequired: false,
    user: toPublicUser(user),
    message: "Successfully logged in to the demo account"
  };
}
...
async function forgotPassword(data) {
  ...
  if (!user) throw new AppError("No user found matching this information", 404);
  await otpService.generateAndSendOtp(user, "RESET_PASSWORD", data.notificationChannel);
  return { userId: user.id, message: `Password reset code sent via ${data.notificationChannel === "SMS" ? "SMS" : "email"}` };
}
