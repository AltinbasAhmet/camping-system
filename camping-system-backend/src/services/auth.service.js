...
async function verifyOtp(userId, code) {
  ...
  if (!['REGISTER', 'LOGIN'].includes(purpose)) {
    throw new AppError("Bu kod giriş doğrulaması için kullanılamaz", 400);
  }
...
async function forgotPassword(data) {
  ...
  if (!user) throw new AppError("Bu bilgilerle eşleşen kullanıcı bulunamadı", 404);
  await otpService.generateAndSendOtp(user, "RESET_PASSWORD", data.notificationChannel);
  return { userId: user.id, message: `Şifre yenileme kodu ${data.notificationChannel === "SMS" ? "SMS" : "e-posta"} ile gönderildi` };
}

async function resetPassword(data) {
  const { user } = await otpService.verifyOtp(data.userId, data.code, "RESET_PASSWORD");
  await prisma.user.update({ where: { id: user.id }, data: { password: await bcrypt.hash(data.password, 10) } });
