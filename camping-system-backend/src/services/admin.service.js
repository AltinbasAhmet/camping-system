...
async function deleteUser(id) {
  ...
  if (ownedCamps > 0 || reservations > 0 || eventBookings > 0 || staffAssignments > 0) {
    throw new AppError(
      "Bu kullanıcıya bağlı kamp, personel ataması, rezervasyon veya bilet kayıtları var; önce onları silin ya da devredin",
      400
    );
  }
...
async function createStaff(data) {
  ...
  if (duplicate) {
    throw new AppError("Bu e-posta veya telefon numarası zaten kullanılıyor", 400);
  }
...
async function getCampStaff(campId) {
  ...
  if (!camp) throw new AppError("Kamp bulunamadı", 404);
...
async function assignStaffToCamp(campId, userId) {
  ...
  if (!camp) throw new AppError("Kamp bulunamadı", 404);
  if (!user) throw new AppError("Personel bulunamadı", 404);
  if (user.role !== "STAFF") throw new AppError("Yalnızca STAFF rolündeki kullanıcılar atanabilir", 400);

  const existing = await prisma.campStaff.findUnique({
    where: { campId_userId: { campId: camp.id, userId: user.id } }
  });
  if (existing) throw new AppError("Bu personel zaten bu kampa atanmış", 400);
...
async function removeStaffFromCamp(campId, userId) {
  ...
  return { message: "Personel kamp ataması kaldırıldı" };
}
