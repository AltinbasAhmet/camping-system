...
async function cancelMyReservation(userId, reservationId) {
  ...
  if (!["PENDING", "CONFIRMED"].includes(reservation.status)) {
    throw new AppError("Bu rezervasyon artık iptal edilemez", 400);
  }
  ...
  if (checkInDate <= now) {
    throw new AppError("Başlamış veya geçmiş rezervasyonlar iptal edilemez", 400);
  }

  const hoursUntilCheckIn = (checkInDate.getTime() - now.getTime()) / (60 * 60 * 1000);
  const lateCancellation = hoursUntilCheckIn <= 24;
  ...
  return {
    ...
    message: lateCancellation
      ? "Rezervasyon son 24 saat içinde iptal edildi. İptal koşulları gereği ücretin %50'si tahsil edilir."
      : "Rezervasyon başarıyla iptal edildi. Başlangıca 24 saatten fazla kaldığı için kesinti uygulanmaz."
  };
}
