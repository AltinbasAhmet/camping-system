...
async function createReservation(userId, data) {
  ...
  // PENDING reservations also temporarily block capacity; otherwise
  // multiple requests awaiting owner approval could be approved in a way
  // that exceeds capacity. If the owner rejects it, the reservation becomes
  // CANCELLED and drops out of this calculation automatically.
...
async function respondToReservation(ownerId, reservationId, decision) {
  ...
  // Item 5: ban on retroactive changes. A reservation with a past
  // check-out date can no longer be approved/rejected by the campsite owner.
  if (reservation.checkOutDate < new Date()) {
    throw new AppError(
      "You cannot perform this action on a reservation with a past date",
      400
    );
  }
