...
async function assignStaffToCamp(req, res, next) {
  try {
    ...
    res.status(201).json({ success: true, message: "Staff member assigned to campsite", data: assignment });
  } catch (error) {
    next(error);
  }
}
...
async function runReminders(req, res, next) {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Sent ${result.checkInCount} check-in and ${result.checkOutCount} check-out reminders`,
      data: result
    });
  } catch (error) {
    next(error);
