...
import { Camp, CampReservation } from "@/lib/types";
...
export default function OwnerReservationsPage() {
  ...
  async function fetchInitialData() {
    try {
      ...
      const reservationsResponse = await apiRequest<{
        success: boolean;
  ...
  async function fetchReservations(campId: string) {
    try {
      setLoading(true);
      setError("");

      const path = campId
        ? `/reservations/owner?campId=${campId}`
        : "/reservations/owner";
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-500">
              View reservations made for your camps.
            </p>
          </div>
            ...
            >
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-4">
                ...
                <div className="rounded-2xl bg-emerald-50/60 p-4">
                  ...
                  <p className="mt-1 font-extrabold text-slate-950">
                    {(reservation as any).user?.name || "Unknown"}
                  </p>
                </div>
              </div>

              {(reservation as any).guests?.length > 0 && (
                <div className="mt-5 rounded-2xl bg-emerald-50/60 p-4">
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    {(reservation as any).guests.map((guest: any) => (
                      <div
                        key={guest.id}
