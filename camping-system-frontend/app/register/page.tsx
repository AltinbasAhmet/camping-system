...
type RegisterResponse = { success: boolean; otpRequired: boolean; userId: number; user: User; message: string };
...
export default function RegisterPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (notificationChannel === "EMAIL" && !email) {
        throw new Error("E-posta ile kod almak için e-posta adresinizi girin.");
      }
      if (notificationChannel === "SMS" && !phone) {
        throw new Error("SMS ile kod almak için telefon numaranızı girin.");
      }
  ...
  function handleVerified(result: { token: string | null; pendingApproval: boolean; user: User }) {
    if (result.pendingApproval) {
      ...
      setPendingApprovalMessage(
        "Doğrulama tamamlandı. Kamp sahibi hesabınız ve kamp başvurunuz, sistem yöneticisi gönderdiğiniz bilgileri ve fotoğrafı inceledikten sonra onaylanacak."
      );
      return;
    }

    saveAuth(null, result.user);
      ...
      <div className="w-full max-w-lg rounded-3xl border border-orange-100 bg-white/90 p-8 shadow-xl shadow-orange-100/70 backdrop-blur">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            <p className="text-sm font-black text-slate-900">Doğrulama kodu nereye gelsin?</p>
