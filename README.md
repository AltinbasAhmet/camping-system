...
## SMS + E-posta rezervasyon hatırlatmaları

Rezervasyonun giriş ve çıkış tarihinden bir gün önce sistem artık kullanıcıya hem SMS hem e-posta göndermeyi dener. Günlük görev her gün saat 09:00'da çalışır.

Gerçek e-posta gönderimi için `camping-system-backend/.env` dosyasında aşağıdaki alanları doldurun:
...
Gmail hesabında iki aşamalı doğrulamayı açın ve bir **Uygulama Şifresi** üretin. Normal Gmail şifresini kullanmayın. `docker-compose.yml`, backend `.env` dosyasını artık otomatik olarak konteynıra aktarır.
...
SMTP veya Twilio tanımlı değilse uygulama çökmez; gönderim geliştirme loguna yazılır.

## Opsiyonel kamp giriş / çıkış saatleri
Kamp sahibi yeni kamp eklerken veya kampını düzenlerken giriş ve çıkış saatlerini isteğe bağlı olarak belirleyebilir. Saatler kamp detayında yalnızca girilmişse gösterilir. Veritabanı migration'ı Docker başlangıcında `prisma migrate deploy` ile uygulanır.
